 ...
 */
const useOCSearchApi = ({ tenantId, filters, auth}, config = {}) => {
  const client = useQueryClient();

  const args = tenantId ? { tenantId, filters, auth } : { filters, auth };
  
  const defaultSelect = (data) => {
    if(data.ocs.length > 0)  data.ocs[0].applicationNo = data.ocs[0].applicationNo || [];
      
    return data;
  };

  const { isLoading, error, data, isSuccess,refetch } = useQuery([tenantId, filters, auth, config], () => Digit.OBPSV2Services.ocsearch(args), {
